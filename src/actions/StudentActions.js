import {STUDENT_CHANGE,CREATE_REQUEST,CREATE_REQUEST_SUCCESS,STUDENT_LIST_DATA_SUCCESS,UPDATE_REQUEST_SUCCESS,UPDATE_REQUEST,DELETE_REQUEST_SUCCESS,DELETE_REQUEST} from './types';
...
export const studentCreate=({isim,soyisim,ogrencinumara,sube})=>{
      ...
      return(dispatch)=>{
        ...
        firebase.database().ref(`/kullanicilar/${currentUser.uid}/ogrenciler`).push({isim,soyisim,ogrencinumara,sube})
        .then(()=>{
...
export const studentUpdate=({isim,soyisim,ogrencinumara,sube,uid})=>{
      ...
      return(dispatch)=>{
        ...
        firebase.database().ref(`/kullanicilar/${currentUser.uid}/ogrenciler/${uid}`).set({isim,soyisim,ogrencinumara,sube})
        .then(()=>{
...
export const studentDelete=({uid})=>{
      ...
      return(dispatch)=>{
        ...
        firebase.database().ref(`/kullanicilar/${currentUser.uid}/ogrenciler/${uid}`).remove()
        .then(()=>{
